...
const MonthPatientFolderPage = () => {
    ...
    // Use hooks with month and year filters
    const { data: patientsResponse, isLoading } = usePatientFolders({
        ...
        monthPatientFolder: month ? parseInt(month) : undefined,
        yearPatientFolder: year ? parseInt(year) : undefined
    })
    ...
    const breadcrumbItems = [
        { title: 'Thông tin bệnh nhân', href: '/patient-folder' },
        { title: `Năm ${year}`, href: '/patient-folder' },
        { title: monthNames[parseInt(month || '1') - 1], href: '#' }
    ].map((item, index) => (
    ...
    return (
        <Container size='xl'>
            <Stack gap='lg'>
                {/* Breadcrumbs */}
                <Group>
                    <Button variant='subtle' leftSection={<IconArrowLeft size={16} />} onClick={handleBackToOverview}>
                        Quay lại
                    </Button>
                ...
                <Group justify='space-between'>
                    <Title order={2}>
                        {monthNames[parseInt(month || '1') - 1]} năm {year}
                    </Title>
...
export default MonthPatientFolderPage
